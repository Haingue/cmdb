...
const SideBar = ({ backdrop, isVisible, toggleSideBarVisibility, hideSideBarHandle } : { backdrop?:boolean, isVisible:boolean, toggleSideBarVisibility:EventHandler<MouseEvent>, hideSideBarHandle:EventHandler<MouseEvent> }) => {  
  return (
    <>
        <aside id="sidebar" className={`fixed ease-out delay-150 duration-150 top-0 ${isVisible ? 'left-0' : '-left-full'} lg:left-0 z-20 flex flex-col flex-shrink-0 w-64 h-full pt-16 font-normal lg:flex transition-width`} aria-label="Sidebar">
          ...
          <div className="h-min px-3 py-4 overflow-y-hidden bg-gray-50 dark:bg-gray-800">
            <ul className="pt-4 mt-4 space-y-2 font-medium border-t border-gray-200 dark:border-gray-700">
              <Button label='Close sidebar' onClick={hideSideBarHandle}>
        ...
        { backdrop && <BackDrop isVisible={isVisible} onClick={toggleSideBarVisibility} /> }
    </>
  )
}
